...
const ISSUANCE_FLOW = [
  ...
  [KeyRound, "기관 승인", "체인별 기관 승인 서명"],
  [Layers3, "Sui · Kaia", "Root·발급자·상태 기록"],
  [BadgeCheck, "외부 검증", "QR로 현재 효력 확인"]
];

const PRIVACY_PLACEMENT = [
  [Database, "기관 서버", "학번·이메일·증빙 원문", "업무와 승인 근거"],
  [BadgeCheck, "공개 검증", "PUBLIC 지정 이름·활동 요약·Proof", "현재 공유 동의는 별도 확인 필요"],
  [Layers3, "Sui · Kaia", "Merkle Root·발급자·상태", "독립된 변조 대조 기준"]
];
...
export function JudgeDemoPage() {
  ...
  const credentialRequestVersion = useRef(0);
  ...
  const loadCredential = useCallback(async (rawId) => {
    const version = ++credentialRequestVersion.current;
    ...
    try {
      ...
      if (version !== credentialRequestVersion.current) return;
      ...
      if (version === credentialRequestVersion.current) setCredentialError(getApiErrorMessage(error, "운영 Credential을 검증하지 못했습니다."));
    } finally {
      if (version === credentialRequestVersion.current) setLoadingCredential(false);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    return () => { credentialRequestVersion.current += 1; };
  }, [credentialParam, loadCredential]);

  const presentation = useMemo(
    () => getVerificationPresentation(getOperationalDisplayStatus(credential, evidenceResult), credential?.issuerName),
    [credential, evidenceResult]
  );
  const explorerUrl = evidenceResult?.chain.explorerUrl;
      ...
      <main className={styles.stage}>
            ...
            {credential && (
              <div className={`${styles.liveResult} ${evidenceResult?.verified ? styles.liveValid : styles.liveInvalid}`}>
                ...
                <div className={styles.liveChecks}>
                  <div>{evidenceResult.payloadMatches ? <Check size={17} /> : <CircleAlert size={17} />}<span>서버 발급 내용 검증</span><strong>{evidenceResult.payloadMatches ? "일치" : "확인 필요"}</strong></div>
                  <div>{evidenceResult.leafMatches ? <Check size={17} /> : <CircleAlert size={17} />}<span>Leaf 재계산</span><strong>{evidenceResult.leafMatches ? "일치" : "확인 필요"}</strong></div>
                  <div>{evidenceResult.merkleRootMatches ? <Check size={17} /> : <CircleAlert size={17} />}<span>Proof→Root</span><strong>{evidenceResult.merkleRootMatches ? "일치" : "확인 필요"}</strong></div>
                  <div>{evidenceResult.chainRecorded ? <Check size={17} /> : <CircleAlert size={17} />}<span>{evidenceResult.chain.networkLabel}</span><strong>{evidenceResult.chainRecorded ? "기록·서버 확인" : "확인되지 않음"}</strong></div>
                </div>
                {evidenceResult.issue && <p>{evidenceResult.issue}</p>}
        ...
        {step === 3 && (
          <section className={styles.slide}>
            ...
            <p className={styles.lead}>브라우저가 공개된 여섯 bytes32로 Trekkey V1 leaf를 만들고 Proof를 접어 API가 공개한 기준 Root와 비교합니다. 원문·기관 서명·체인 확정과 최신 상태는 서버 조회 결과입니다. 브라우저가 직접 RPC를 조회하지 않으며, 공개 원장 링크로 추가 확인할 수 있습니다.</p>
            <div className={styles.proofEquation}>
              ...
              <ArrowRight size={22} />
              <article><small>PUBLIC MERKLE ROOT</small><strong>{compact(evidenceResult?.calculatedRoot || "Root 대기")}</strong></article>
            </div>
            <div className={styles.proofVerdict}>
              {evidenceResult?.verified ? <ShieldCheck size={28} /> : <Gauge size={28} />}
              <div><span>브라우저 Proof 판정</span><h2>{evidenceResult ? evidenceResult.verified ? "서버 검증 결과와 브라우저 Proof 재계산이 일치합니다" : evidenceResult.issue : "앞 단계에서 운영 Credential을 먼저 선택하세요"}</h2></div>
            </div>
        ...
        {step === 4 && (
          <section className={styles.slide}>
            ...
            <h1>원문은 체인에 저장하지 않고<br /><em>PUBLIC 지정 요약은 별도로 표시합니다</em></h1>
            ...
            <div className={styles.evidenceStrip}>
              <article><strong>서버</strong><span>실행 결과는 릴리스별 보고서 기준</span></article>
              <article><strong>Move · Solidity</strong><span>Root·기관 승인·상태 전이 검사</span></article>
              <article><strong>프런트</strong><span>API·Proof·사용 흐름 회귀 검사</span></article>
              <article><strong>1·10·100·500·1,000</strong><span>브라우저 측정 대상 배치</span></article>
            </div>
        ...
        {step === 5 && (
          <section className={`${styles.slide} ${styles.finalSlide}`}>
            ...
            <div className={styles.roadmap}>
              <span>운영 경계</span>
              <p><strong>폐기·대체</strong>는 관리자 원장의 기관 승인 작업으로 처리합니다. 시연의 합성 데이터와 실제 운영 데이터, 격리 테스트 결과와 운영 배포 상태는 구분해야 합니다.</p>
            </div>
